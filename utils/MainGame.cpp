...
void MainGameClass::GenerateBlincy() {
	while (1) {
		Blinky.x = static_cast<float>(rand() % map_width);
		Blinky.y = static_cast<float>(rand() % map_height);
		if (CheckWall(static_cast<int>(Blinky.x), static_cast<int>(Blinky.y)) && Blinky.CalculateDistance(player.x, player.y) > 5) {
			map[static_cast<int>(Blinky.y) * map_width + static_cast<int>(Blinky.x)] = 'B';
			break;
		}
	}
}
...
void MainGameClass::GenerateTinky() {
	while (1) {
		Tinky.x = static_cast<float>(rand() % map_width);
		Tinky.y = static_cast<float>(rand() % map_height);
		if (CheckWall((int)Tinky.x, (int)Tinky.y) && Blinky.CalculateDistance(player.x, player.y) > 5) {
			map[static_cast<int>(Tinky.y) * map_width + static_cast<int>(Tinky.x)] = 'T';
			break;
		}
	}
}
...
void MainGameClass::GeneratePlayerPos() {
	player.x = static_cast<float>(rand() % map_width);
	player.y = static_cast<float>(rand() % map_height);
	while (map[static_cast<int>(player.y) * map_width + static_cast<int>(player.x)] == '#' || Blinky.CalculateDistance(player.x, player.y) <= 15) {
		player.x = static_cast<float>(rand() % map_width);
		player.y = static_cast<float>(rand() % map_height);
	}
}
...
void MainGameClass::PlayerTrunAndMove() {
	...
	// 向左平移移动（D 键）
	if (IS_KEY_DOWN(0x44)) {
		player.x += cosf(player.angle) * 5.0f * elapsed_timeS;
		player.y -= sinf(player.angle) * 5.0f * elapsed_timeS;
		// 碰撞检测
		if (map[(int)player.y * map_width + (int)player.x] == '#') {
			player.x -= cosf(player.angle) * 5.0f * elapsed_timeS;
			player.y += sinf(player.angle) * 5.0f * elapsed_timeS;
...
void MainGameClass::PlayerRaySimulation() {
	for (int x = 0; x < screen_width; x++) {
		...
		int ceiling = static_cast<int>((float)(screen_height / 2.0f) - (screen_height / 1.5f) / ((float)distance_to_wall));
		int floor = screen_height - ceiling;
		//计算猎人的高度
		int Blinky_ceiling = static_cast<int>((float)(screen_height / 2.0f) - (screen_height) / ((float)distance_to_Blinky));
		int Blinky_floor = static_cast<int>((float)(screen_height * 5.0f / 6.0f) - ((float)distance_to_Blinky));
		int Tinky_ceiling = static_cast<int>((float)(screen_height / 2.0f) - (screen_height) / ((float)distance_to_Tinky));
		int Tinky_floor = static_cast<int>((float)(screen_height * 5.0f / 6.0f) - ((float)distance_to_Tinky));
		//计算信息碎片的高度
		int info_ceiling = static_cast<int>((float)(screen_height * 3.0f / 4.0f));
		int info_floor = static_cast<int>((float)(screen_height * 6.0f / 7.0f) - (float)distance_to_info);
...
void MainGameClass::DrawGamePlaying() {
	//显示存活时间
	swprintf_s(screen, 177, L"X=%3.2f  Y=%3.2f  FPS=%3.2f  InfoFragment=%d  Punch=%d  Map=%d  LiveTime=%llds", player.x, player.y, 1.0f / elapsed_timeS, num_info_fragment, player.is_punch, can_open_map, live_time.count());
	...
	WriteConsoleOutputCharacterW(*renderer.h_p_output, screen, screen_width * screen_height, { 0,0 }, &dw_byte_wirtten);
}
...
void MainGameClass::ShowMap() {
	...
	while (caculate_angle < 0)
		caculate_angle += static_cast<float>(6.28);
	while (caculate_angle >= 6.28)
		caculate_angle -= static_cast<float>(6.28);
	float direction = static_cast<float>(fabs(caculate_angle) / 1.570796);
	if (direction < 0.5) {
	...
	if (IS_KEY_DOWN(0x4D) && can_open_map) {
		...
		COORD player_char_pos = { static_cast<SHORT>(player.x), static_cast<SHORT>(player.y + 1) };
		COORD char_size = { 1,1 };
		//没啥用，但是必须要有
		DWORD dw_byte_wirtten = 0;
		...
		if (!Blinky.is_death) {
			...
			COORD Blinky_char_pos = { static_cast<SHORT>(Blinky.x) ,static_cast<SHORT>(Blinky.y + 1) };
		...
		if (!Tinky.is_death) {
			...
			COORD Tinky_char_pos = { static_cast<SHORT>(Tinky.x) ,static_cast<SHORT>(Tinky.y + 1) };
	...
	if (!Blinky.is_death) {
		...
		if (num_info_fragment == 0) {
			...
			if (Blinky.run_thinking > 180) {
				...
				while (map[new_y * map_width + new_x] == '#' && sqrtf(powf(player.x - new_x, 2) + powf(player.y - new_y, 2)) < 8) {
	...
	if (!Tinky.is_death) {
		if (num_info_fragment != 0) {
			...
			if (Tinky.thinking > 300) {
				...
				target.x = static_cast<long>(player.x), target.y = static_cast<long>(player.y);
		...
		if (num_info_fragment == 0) {
			...
			if (Tinky.run_thinking > 180) {
				...
				int new_x = rand() % map_width;
				int new_y = rand() % map_height;
				while (map[new_y * map_width + new_x] == '#' && sqrtf(powf(player.x - new_x, 2) + powf(player.y - new_y, 2)) < 10) {
					new_x = rand() % map_width;
					new_y = rand() % map_height;
				}
				Node* end = new Node(new_x, new_y);
		...
		if (num_info_fragment != 0) {
			...
			if (((int)Blinky.x == (int)player.x && (int)Blinky.y == (int)player.y)) {
				if (num_hunter == 2)
					...
					num_hunter = 0;
				map[static_cast<int>(Blinky.y) * map_width + static_cast<int>(Blinky.x)] = ' ';
				Sleep(200);
			...
			if (((int)Tinky.x == (int)player.x && (int)Tinky.y == (int)player.y)) {
				if (num_hunter == 2)
					...
					num_hunter = 0;
				map[static_cast<int>(Tinky.y) * map_width + static_cast<int>(Tinky.x)] = ' ';
				Sleep(200);
				Tinky.is_death = true;
				Tinky.x = 0;
				Tinky.y = 0;
...
void MainGameClass::HandleMouseMovement() {
	// 获取控制台窗口句柄
	...
	// 计算控制台窗口中心坐标
	int centerX = (rectConsole.left + rectConsole.right) / 2;
	int centerY = (rectConsole.top + rectConsole.bottom) / 2;
	...
	// 计算鼠标移动的偏移量
	int deltaX = cursorPos.x - centerPos.x;

	// 调整玩家视角，假设每个像素移动对应的角度变化为 0.005f
	player.angle += deltaX * 0.002f;

	// 重置鼠标光标到控制台窗口中心
	SetCursorPos(centerPos.x, centerPos.y);
}

void MainGameClass::LockAndHideCursor() {
	// 获取控制台窗口句柄
