...
Menu::Menu()
{
	this->strat_filename = "ASCII/menu_start.txt";
	this->help_filename = "ASCII/menu_help.txt";
	this->exit_filename = "ASCII/menu_exit.txt";
	this->return_menu_filename = "ASCII/game_over_menu.txt";
	this->over_exit_filename = "ASCII/game_over_exit.txt";
...
void Menu::UpdateAndRenderMenu()
{
	...
	case GameState::StartMenu::GS_StartSelect:
	{
		...
		{
			//进入游戏前停止播放菜单音乐
			mciSendStringW(L"stop music/menu_bgm.mp3", 0, 0, 0);
			mciSendStringW(L"open music/game_bgm.mp3", 0, 0, 0);
			mciSendStringW(L"play music/game_bgm.mp3 repeat", 0, 0, 0);
...
void Menu::ShowStartMenu()
{
	...
	if (!infile.is_open())
	{
		MessageBoxW(0, L"读取开始游戏菜单失败", 0, 0);
		return;
	}
	int index = 0;
	std::string str;
...
void Menu::ShowExitMenu()
{
	...
	if (!infile.is_open())
	{
		MessageBoxW(0, L"读取退出游戏菜单失败", 0, 0);
		return;
	}
	int index = 0;
	std::string str;
...
void Menu::HelpMenuComplement()
{
	...
	std::ifstream infile("help.txt", std::ios::in);
	std::string str;
	int index = 0;
	while (getline(infile, str))
	{
