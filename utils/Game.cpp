...
void Game::InitGlobal() {
	...
	mciSendStringW(L"open music/menu_bgm.mp3", 0, 0, 0);
	mciSendStringW(L"play music/menu_bgm.mp3 repeat", 0, 0, 0);
...
//游戏结束菜单
void Game::GameOver() {
	mciSendStringW(L"close music/game_bgm.mp3", 0, 0, 0);
	//播放音乐
	if (is_play_menu_music == false && is_player_die == false) {
		mciSendStringW(L"playmusic/menu_bgm.mp3 repeat", 0, 0, 0);
		is_play_menu_music = true;
	}
